    ...
    methods
        ...
        function Start(obj)
            ...
            if obj.evaluated <= 0
                ...
                labSend(obj.problem, 2, obj.count);
                labSend(obj.problem, 3, obj.count);
        ...
        function notermination = NotTermination(obj, Population)
            ...
            if obj.evaluated >= index * obj.evaluation / num
                ...
                tmp = inf(size(objs, 1), length(obj.DM));
                obj.count = obj.count + 1;
                labSend(Population, 2, obj.count);
                labSend(Population, 3, obj.count);
                % labBarrier;
                data1 = labReceive(2, obj.count);
                tmp(:, 1) = data1{1};
                data2 = labReceive(3, obj.count);
                tmp(:, 2) = data2{1};
                ...
                decs = obj.result{end}.decs;
                % save(sprintf('%s_%s.mat',func2str(obj.algorithm), class(obj.problem)), 'decs');
                
                % Metrics = obj.problem.Metric;
                % Score = cellfun(@(S)GLOBAL.Metric(S, Population, obj.PF, obj.DM), Metrics, 'UniformOutput', false);
                Metrics = {@IGD1};
                Score = cellfun(@(S)GLOBAL.Metric(S, Population, obj.Opt, obj.DM), Metrics, 'UniformOutput', false);
